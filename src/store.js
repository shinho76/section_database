...
export const useStore = create((set, get) => ({
  ...
  // Length defaults to 10m regardless of which page adds the item (a
  // reasonable placeholder the user is expected to correct per project) -
  // still overridable per-row in BomModal, which still flags a row red if
  // the user clears it down to blank/invalid.
  addToBom: (item) => set((s) => {
    const bom = [...s.bom, { id: bomIdSeq++, qty: 1, ...item, lengthM: item.lengthM ?? 10 }];
    saveBom(bom);
    return { bom };
  }),
