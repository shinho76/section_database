  ...
  return (
    <div className="home-view">
      ...
      <section className="home-section">
        ...
        <p>
          상단 검색창 하나로 <b>AISC·KS 전체 규격과 모든 참고 자재(강판·배근재·환봉·패스너 등)를
          동시에</b> 검색합니다. 사이드바에서 카테고리를 일일이 눌러 들어갈 필요 없이, AISC 라벨
          ("W24X76" 같은)이든 KS 규격 표기("H-400X200")든 아는 표기 그대로 입력하면 즉시 결과가
          뜨고, 각 결과 행에 AISC 라벨과 대응하는 KS 규격이 나란히 표시됩니다. 도면·계산서를
          검토하다 낯선 규격명이 나왔을 때, 원하는 자재를 몇 초 안에 찾아내는 데 초점을 맞췄습니다.
        </p>
      </section>
      ...
      <section className="home-section">
        ...
        <p>
          설계 도중 자재를 AISC에서 KS로(또는 그 반대로) 바꿔야 하는 상황은 흔하지만, "이 부재와
          가장 가까운 규격이 뭔지"를 매번 표로 대조하기는 번거롭습니다. 이 사이트는 목록에서
          <b>가장 근접한 상대 규격으로 바로 연결되는 링크</b>를 제공해, 클릭 한 번으로 단면
          치수·형상(SVG)·단위중량뿐 아니라 A(단면적)·Ix·Sx·Zx·rx·Iy·ry·J 같은 단면성능까지
          두 규격을 나란히 비교해서 보여줍니다. 치수만 비슷하고 성능은 부족한 "가짜 대체재"를
          거르고, 규격 변경 시에도 어떤 자재가 실제로 등가인지 즉시 파악할 수 있게 한 것이
          이 기능의 핵심입니다.
        </p>
      </section>
