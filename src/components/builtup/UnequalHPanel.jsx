...
export default function UnequalHPanel() {
  ...
  const svgUs = valid ? drawUnequalHSVG(dimsIn, '"', propsIn.W.toFixed(1) + ' lb/ft', propsIn.A.toFixed(2) + ' in²') : null;
  const svgMm = valid ? drawUnequalHSVG(mm, 'mm', (propsIn.W * LBFT_TO_KGM).toFixed(1) + ' kg/m', (propsIn.A * IN2_TO_MM2).toFixed(0) + ' mm²') : null;
      ...
      <div className="bh-grid">
        <div className="panel">
          ...
          <div className="field-row">
            <DualUnitInput label="d (전체 높이)" mm={mm.d} onChangeMm={set('d')} />
          </div>
          <div className="field-row">
            <label>tw (웹 두께)<ThicknessCombo value={mm.tw} onChange={set('tw')} /></label>
          </div>
          <div className="field-row">
            <DualUnitInput label="bf-top (상부 플랜지 폭)" mm={mm.bfTop} onChangeMm={set('bfTop')} />
            <label>tf-top (상부 플랜지 두께)<ThicknessCombo value={mm.tfTop} onChange={set('tfTop')} /></label>
          </div>
          <div className="field-row">
            <DualUnitInput label="bf-bot (하부 플랜지 폭)" mm={mm.bfBot} onChangeMm={set('bfBot')} />
            <label>tf-bot (하부 플랜지 두께)<ThicknessCombo value={mm.tfBot} onChange={set('tfBot')} /></label>
          </div>
        ...
        <>
          ...
          <div className="panel">
            ...
            <table className="props">
              ...
              <tbody>
                <tr><td className="sym mono">A</td><td className="r mono">{propsIn.A.toFixed(2)} <em>in²</em></td><td className="r mono">{(propsIn.A * IN2_TO_MM2).toFixed(0)} <em>mm²</em></td><td className="desc">단면적</td></tr>
                ...
                <tr><td className="sym mono">W</td><td className="r mono">{propsIn.W.toFixed(1)} <em>lb/ft</em></td><td className="r mono">{(propsIn.W * LBFT_TO_KGM).toFixed(1)} <em>kg/m</em></td><td className="desc">단위중량</td></tr>
              </tbody>
            </table>
